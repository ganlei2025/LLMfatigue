...
def linear_calculate(func_terms, func_strs, lhs, add_const, mode):
    print("\n================= [DEBUG] linear_calculate =================")
    print("[func_strs] =", func_strs)
    ...
    if add_const:
        try:
            ...
            print("[ERROR] add_const failed:", e)
            print("[func_terms] =", func_terms)
            return func_strs, 0, 0, False, "false_shape"
    ...
    try:
        if mode == 'sparse_regression':
            print("[DEBUG] using sparse_regression")
            y_rhs, coef, valid, error_type = sparse_calculate(func_terms, lhs)

        elif mode == 'regression':
            print("[DEBUG] using regression")
            y_rhs, coef, valid, error_type = coef_calculate(func_terms, lhs)

        else:
            print("[ERROR] invalid mode:", mode)
            return func_strs, 0, 0, False, "invalid_mode"

    except Exception as e:
        print("\n*************** [Evaluator ERROR in linear_calculate] ***************")
        print("[Expression strings] =", func_strs)
        print("[func_terms] =", func_terms)
        print("[lhs shape] =", lhs.shape)
        print("[Error] =", e)
        print("*******************************************************************\n")
        return func_strs, 0, 0, False, "linear_exception"

    # ---------- Step 3: 打印结果 ----------
    print("[DEBUG] valid =", valid, ", error_type =", error_type)
    print("====================================================================\n")
...
def sparse_calculate(func_terms, lhs, l0_penalty=10**-5):
    ...
    if error_type == 'lstsq_error':
        print("\n[Evaluator ERROR] sparse_calculate -> lstsq_error")
        print("[func_terms]:", func_terms)
        print("[lhs shape]:", lhs.shape)
