...
def main():
    ...
    parser.add_argument('--LLM_name', type=str, default="deepseek-7b", help='utilized llm model')
    ...
    parser.add_argument('--operators', type=str, default="{+, -, *, /, ^2}", help='operators library')
    ...
    parser.add_argument('--sort', type=str, default="not_reverse", choices=['not_reverse', 'reverse', 'Not sorted'],
                        help='sort or not')
    ...
    parser.add_argument('--metric_params', nargs='+', type=float, default=[0.04])
    ...
    for i in range(args.max_epoch):
        ...
        info_str, info_score = organize(optimizer_input, cur_optimize_type, args.sort, evaluator.pq,
                                        use_pqt=args.use_pqt, ode=ODE_flag)
        ...
        total_request_time += dur_time
        print(
            f"{cur_optimize_type}:  best expression: {str(best_sample_cur)}, score: {best_sample_cur.score}, duration: {time() - begin_time}")
